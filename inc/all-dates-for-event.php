...
if ($eventrecord['schedule-type'] === 'dates') {   
    ...
    echo "<b>Scheduled Dates</b>";  
        
    foreach ($datearray as $id => $item) {         
        $itemyear = substr ($item, 0,4);
        $itemmonth = substr ($item, 5,2);
        $itemday = substr ($item, 8,2);
      
        $string = date("l F d, Y", mktime(0,0,0,$itemmonth, $itemday, $itemyear));
        echo "<br>&nbsp;&nbsp;  " . $string;   
        if ($id === 0) {
            $startday = $string;
        }
        ...
        $dayofweek  = date("l", mktime(0,0,0,$itemmonth, $itemday, $itemyear));  
    }
